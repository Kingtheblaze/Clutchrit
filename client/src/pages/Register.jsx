...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const res = await register(username, password, role, inviteCode);
    if (res.success) {
      navigate('/admin');
    } else {
    ...
    <div className="min-h-screen w-full bg-void flex border-t border-border">
      ...
      <div className="hidden lg:flex w-[60%] relative overflow-hidden bg-void items-center justify-center">
        ...
        <svg className="absolute inset-0 w-full h-full pointer-events-none opacity-20" viewBox="0 0 1000 1000" preserveAspectRatio="xMidYMid slice">
          <circle cx="800" cy="200" r="300" stroke="var(--acid)" strokeWidth="1" fill="none" strokeDasharray="5 15" />
          <rect x="600" y="200" width="200" height="400" stroke="var(--acid)" strokeWidth="2" fill="none" />
          <path d="M1000,1000 L0,0" stroke="var(--acid)" strokeWidth="1" strokeOpacity="0.5" />
        </svg>

        {/* Floating Text Fragments */}
        <div className="absolute top-[20%] right-[20%] font-mono text-[10px] text-acid opacity-20">SYSTEM // RECRUITMENT</div>
        <div className="absolute bottom-[30%] left-[30%] font-mono text-[10px] text-acid opacity-40">CONNECTION :: PENDING</div>
        <div className="absolute top-[60%] right-[40%] font-mono text-[10px] text-acid opacity-20">ENCRYPTION : ACTIVE</div>
        <div className="absolute top-[10%] left-[10%] font-mono text-xs text-acid opacity-30">NEW NODE DETECTED</div>
      </div>
        ...
        <div className="max-w-md w-full mx-auto my-auto py-8">
          ...
          <div className="mb-8">
            <div className="flex items-center gap-2 mb-4">
              ...
              <p className="font-subheading text-acid uppercase text-xs tracking-[0.2em]">ADMIN RECRUITMENT</p>
            </div>
            <h1 className="font-heading text-6xl md:text-7xl leading-none text-text-0 mb-[-10px]">NEW</h1>
            <h1 className="font-heading text-6xl md:text-7xl leading-none text-acid" style={{ textShadow: 'var(--acid-glow)' }}>ADMIN</h1>
          ...
          >
            ...
            <div className="flex flex-col gap-2">
              ...
              <select
                ...
                className="bg-surface-2 border border-border text-text-0 p-[12px_16px] font-mono text-sm focus:border-acid focus:outline-none transition-colors cursor-pointer"
              >
                <option value="admin">CLUB MEMBER</option>
                <option value="super_admin">ROOT OVERSEER</option>
              </select>
            </div>

            <div className="flex flex-col gap-2">
              <label className="font-subheading text-text-1 text-[10px] uppercase tracking-widest">INVITE CODE</label>
              <input
                type="text"
                value={inviteCode}
            ...
            {error && (
              <div className="text-acid font-mono text-xs">
                {`> ${error}`}
              </div>
            )}
          ...
          <div className="mt-12 pt-8 border-t border-border border-dashed text-text-2 font-mono text-[10px] uppercase relative">
            <p className="opacity-50">REGISTRATION GRANTS MEMBER PRIVILEGES TO CLUTCHRIT SYSTEM.</p>
          </div>
        </div>
      </div>
    </div>
